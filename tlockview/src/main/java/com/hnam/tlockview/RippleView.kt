...
class RippleView(context: Context?, private val attrs: AttributeSet?) : RelativeLayout(context, attrs) {
    ...
    private fun randomAnimationDuration() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ThreadLocalRandom.current().nextInt(MIN_RIPPLE_DURATION, MAX_RIPPLE_DURATION)
    } else {
